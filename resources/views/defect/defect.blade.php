...
@endsection
@section('title', 'Maintenance | Kerusakan')
@section('defectActive', 'active')
	...
	<div class="col-sm-12">
		<div class="card">
			<div class="card-body">
				...
				@endif
				<p class="card-title">Tabel Kerusakan Barang pada Kamar</p>
				
				@if( !empty($defects[0]->id) )
				<!-- table -->
				<table id="tablePengaduan" class="table table-stripped table-responsive" width="100%" cellspacing="0">
			        ...
			        <tbody>
			            ...
			            <tr>
			                ...
			                <td>{{$defect->number}}</td>
			                <td>{{ $defect->name }}</td>
			                <td>{{date("M jS, Y H:i:s", strtotime($defect->created_at))}}</td>
			                <td class="@if($defect->d_status == 0)
										belum
									@else
										sudah
									@endif">
			                	<span class="la @if($defect->d_status == 0)
			                ...
			                <td>
			                	<div data-toggle="tooltip" data-placement="top" title="Detail pengaduan">
				                	<button type="button" class="btn btn-success" onclick="showDefect({{ $defect->id }})">
				                		<span class="fa fa-eye"></span>
				                	</button>
				                </div>
				                <div data-toggle="tooltip" data-placement="top" title="Perbaiki Sekarang">
				                	<button type="button" class="btn btn-primary btn-edit-jadwal" onclick="fixit({{$defect->id}})" @if( $defect->d_status ==1 ) disabled @endif>
				                ...
				                <div data-toggle="tooltip" data-placement="top" title="Hapus pengaduan">
				                	...
				                	<form action="/defect/{{$defect->id}}" method="POST" style="display: none;" id="delete-form">
											{{csrf_field()}}
											{{method_field('DELETE')}}
									</form>
				                </div>
			    ...
			    <!-- table -->
			@else
				<center>Belum ada kerusakan pada kamar</center>
			@endif
...
<!-- modal show kerusakan -->
<div class="modal modal-large fade" id="modalDefect" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Detail Kerusakan</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body" id="showdefect">
...
<script>
	function hapus(){
		swal({
		  title: "Anda yakin?",
		  text: "Kerusakan akan terhapus secara permanen!",
		  icon: "warning",
		  buttons: true,
		  dangerMode: true,
		})
		.then((willDelete) => {
		  if (willDelete) {
		    swal({
		    	title: "Kerusakan Terhapus!",
				text: "Hapus kerusakan berhasil!",
				icon: "success",
				button: "OK",
		    })
			.then((value) => {
		    ...
		    swal("Batal Menghapus Kerusakan");
		  }
		});
...
@section('script')
	...
	function showDefect(id){
		var link = "{{url('defect/')}}/"+id;
		$.ajax({
			type: 'get',
			url: link,
			success: function(data){
				$('#showdefect').html(data);
